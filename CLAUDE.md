...
| `26_get_off_my_ship` | 《내 배에서 내려》 / GET OFF MY SHIP | **러프 기획안 v4 (2026-08-11 · 숏폼 리듬)** — 트리트먼트 = 비트별 줄바꿈·초단문·대사 견인(소설 문단체 기각). 타이틀 확정(사용자). — 글로벌 여성향·복수/권선징악 메인·**1910s 초호화 여객선(콜 라인 기함 임페리얼호 처녀출항 = '타이타닉' 연상)**·EN 발화·50화 내외·무료 8(제안). **원안 = `C:\Users\Rowan\Downloads\내가 세우고 내가 무너뜨린다.역대본.md`**(NetShort 50화) — **구조·비트 순서·클리프·감정 곡선 1:1 + 행위는 복사 금지·배 것으로 치환**(사용자 룰: 기능 동일·행위만 다르게·감정은 유치할 정도로 직관): 팁 조롱→밀항자 몰이(8화 처형 명분 콜백)·얼음물→샴페인 버킷·전기뱀장어→밤바다 뱃전 매달기. v1 금박시대 호텔안 = 대중 인지 부족 기각(`_p0_rough_v1_gilded.md` 보존). 인물 = 이블린 윈스럽(해운 상속녀·임신)/줄리언 콜(남편·후회남·같은 배 위층 원격 공모)/델라(정부)/마사 콜(시어머니·며느리 편·3등칸 축출). 셀링포인트 = 3줄 규격(타이타닉 연상·30-50 직빵·밀항자 엔진). 무료 8화 컷 = 남편이 젖은 여자 턱 들어 올려 얼굴 확인 프리즈. 타이틀 = 최종 사이다 대사 겸용. 정본 = `26_get_off_my_ship_p0_rough.md`·이력 = `26_get_off_my_ship_00_meta.md`. 잔여 = 사용자 채택 판정 → phase_p |
