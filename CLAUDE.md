...
| `19_archmage_executed` | MY KILLER CALLS ME SON / 나를 죽인 남자가 날 아들이라 부른다 | **러프 기획안 v8 (2026-07-30)** — **v8 = 초직관 사이다 골격(사용자 제시안 채택): 서류 청산 전량 폐기(각서·도면·피서명·조서 = 소송극)·숨기는 이유 없음·이해시간 0초·1회 1사이다.** 1화 = 마왕 목 벤 등 뒤 배신 + 지팡이 밟기·훈장 떼기 농락 → 절벽 → 원수 아들 몸 리스폰 → 손가락 튕기니 마력 그대로. 2화 파혼 뒤집기 / 3화 지팡이가 주인 알아보고 원수 공격 / 4화 "내가 만들었으니까" / 5화 드라이든 흙바닥 / 6화 원수 머리 짓밟기 / 7화 왕도 상공 학살쇼 / 8화 **오르반 단독 통보**("십 년 동안 내 이름 가지고 놀아보니 재미있었어?"). **메인이 수정한 4건 = ①8화 셋 동시 선포 → 오르반 1인(페이월 자본 보존·사냥개 구도) ②약혼자 손목 꺾기 → 술잔 정지+서류 소각(되돌아와 매달리는 상품 보존·NA 리스크) ③7화 대군 증발 → 잔당만, 본체는 눈만 뜸 ④1화 첫 타격 대상 하녀·경비 → 사촌 패거리.** 이하 폐기 이력 — — 남성향 회귀+Hidden Identity+배신 정산·글로벌 EN·50화 내외·무료 8(제안)·오리지널. 컨셉 이미지 8장(`inbox/fantasy_huge_scale_perfect`) 발주. **1화 = 균열 안에서 처절 농락→살해→회귀**(농락 대화가 세계관·관계·힘·이유·감정 전량 운반 / 농락 = 자기 봉인 도면에 자기 피로 양도 각서 강제 서명 → 그 종이 = 오르반 정통성·회귀 통로·8화 회수물). **귀환 = 자기를 죽인 마탑주의 하나뿐인 아들 몸(렌 오르반 19)** — 파혼당한 저택 제일 우스운 남자. 엔진 = 원수의 이름·자원으로 원수의 자리 뺏기, 정체만 감추고 힘은 2화부터 전량 공개. 무료런 공간 2곳(균열 1화 + 저택 연회 2~8화 하룻밤). 세라핀 = 목격자 아니라 **파혼한 약혼자**(그 밤 안에 되돌아옴). **v1~v6 폐기 사유(반복 금지):** 짐꾼·잡역·빗자루 밑바닥 = 고구마 / 조사하는 눈이 화면 채우면 미스터리 질감 / 1화 프롤로그 몽타주·상대가 세계(제도·균열) = 숏폼 아님 / 공간 기능 원장·클리프 형태표 = 사용자 감각을 법칙화한 개소리. 기각 외부안 = 아르덴 모방 학파·사막 아티팩트 배후 미스터리·정체 윙크 대사. 산출 = `19_archmage_executed_p0_rough.md`. 잔여 = 사용자 채택 판정 → phase_p 진입 게이트·0단계 문답 → 정식 제출본 |
