...
| `26_get_off_my_ship` | 《내 배에서 내려》 / GET OFF MY SHIP | **러프 기획안 v2 (2026-08-11 · 여객선안)** — 글로벌 여성향·복수/권선징악 메인·**1910s 초호화 여객선(콜 라인 기함 임페리얼호 처녀출항)**·EN 발화·50화 내외·무료 8(제안). **원안 = `C:\Users\Rowan\Downloads\내가 세우고 내가 무너뜨린다.역대본.md`**(NetShort 50화) — 무료런 굴욕 적립 구조·클리프 형태·킬러 대사 1:1 이식, 세계만 치환(사용자: 감정·구조 보존 + 대중이 설명 없이 아는 세계 — v1 금박시대 호텔안은 대중 인지 부족으로 기각, `_p0_rough_v1_gilded.md` 보존). 인물 = 이블린 윈스럽(해운 왕조 상속녀·임신)/줄리언 콜(남편·후회남·같은 배 위층서 원격 공모)/델라(정부·1등석 안주인 행세)/마사 콜(시어머니·며느리 편·3등칸 축출). 무대 자산 = 출항 후 못 내림(컨테이너 물리 완성)·1등석↔3등칸 계급 낙차·전기뱀장어 수조 = 팜코트 처녀출항 구경거리. 무료 8화 컷 = 남편이 젖은 여자 턱을 들어 올려 얼굴 확인 프리즈. 타이틀 = 최종 사이다 대사 겸용. 정본 = `26_get_off_my_ship_p0_rough.md`·이력 = `26_get_off_my_ship_00_meta.md`. 잔여 = 사용자 채택 판정 → phase_p·타이틀 확정 |
