...
| `21_black_hong_gildong` | 《흑길동》 / BLACK GILDONG | **러프 기획안 v6 (2026-07-31)** — 톤 = **액션 병맛 활극**(인물 전원 진지·예법 유지·웃는 건 화면 밖뿐). 병맛 설정 3 = 총알은 양반집 **은수저**를 녹인 것(얼자가 은수저로 쏨) / 양반은 마늘을 못 먹어 반가 밥상엔 백김치만(길동은 아침마다 마늘 씹음) / 조정 회의는 원래 밤에 열림(야간 조회가 관례). **한은 3곳만 정극** = 4화 "네 어미가 종이다·네가 뛰어난 것이 이 집에서 가장 큰 죄다" / 7화 방에 적힌 '이름 없는 도적' / 8화 열아홉 해 만에 처음 듣는 호칭. 홍길동 요소 = 노비 어머니·얼자·호부호형·재능 있어도 막힌 벼슬길·활빈당·분신술만(딥컷 제외). 이하 — — 한국 남성향·히든 아이덴티티(낮/밤 이중생활)·사이다 액션활극·KR 발화·50화 내외·무료 8(제안)·홍길동전 각색. **출처 = 인터넷 밈 〈흑길동〉(2019~·블레이드 리터칭) — 밈의 알맹이 = "탐관오리가 백성의 고혈을 빤다"는 관용구가 비유가 아니라 사실.** 블레이드 설정 × 홍길동전 골격 1:1 맞물림 = 아버지를 아버지라 못 부르는 이유가 그 아버지가 삼백 년 순혈 흡혈귀라서·그는 인간 어미에게서 난 반쪽이라 대낮에도 다님. 핵심 쾌감 = **낮에 이마 땅에 대고 절한 상대의 목을 그날 밤에 벤다**(매 화 같은 형태·낮에 절한 상대만 교체). 톤 = 전원 진지·아무도 안 웃음(웃음의 대상 = 관용구의 문자화이지 그의 몸이 아님). 공간 2곳 = 홍판서 댁 + 고을 관아. 페이월 = 대전 창문이 전부 닫힌 대낮, 임금이 "잘 왔다, 홍판서의 아들" 하고 대신들 눈이 전부 붉음. **v1(백분 기믹) v2(분신술) 폐기 — 피부를 위장 장치로 쓰면 blackface 거울상·완력 중심은 brute 스테레오타입·분신술은 밈 핵심 아님.** 산출 = `21_black_hong_gildong_p0_rough.md`. 잔여 = 사용자 채택 판정 → phase_p · 기존작 대조(2022 웹툰 《은탄》·2025 AI 뮤비 《미스터홍》) |
