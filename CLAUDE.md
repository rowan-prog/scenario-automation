...
| **플랫폼 기획안 (phase_p)** | `prompts/phase_p_platform_proposal.md` — 대원칙 5(원안=정본 verbatim·로그라인 5요소·갈등축 발명 금지·§C-4-2/§C-2-2 게이트·룰 정합) → 기계 게이트 6종 → 빌더 `tools/build_vigloo_proposal.py` (2026-07-16 제정 · 실증 = My Million-Dollar Reset) | 창작 코어·트리트먼트 verbatim 보강 = opus / 시놉·캐릭터 = sonnet / 메타 = haiku 병렬 → 머지·docx 조립 = 메인 |
