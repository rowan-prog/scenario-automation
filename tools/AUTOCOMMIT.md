...
두 스크립트 모두 커밋 **직전에** 개인 폴더의 자산을 저장소로 복사합니다. 안 그러면 사본이 낡습니다.

| 자산 | 정본 | 저장소 사본 |
|---|---|---|
| 에이전트 14종 | `agents/` (저장소) | — 저장소가 정본. 고치면 `--to-home` 으로 내림 |
| 메모리 236개 | `~/.claude/projects/.../memory/` | `memory/` — **백업 사본. 직접 편집 금지** |

수동 확인·동기화:

```
python tools/sync_claude_assets.py                 차이만 확인
...
- 로그온 상태에서만 돕니다. 로그아웃 중에는 안 돕니다(꺼져 있었으면 다음 로그온 때 따라잡습니다).
- 아직 백업 안 되는 것: **훅 스크립트 3종**(`~/.claude/hooks/`), `inbox/`, 로그 파일.
