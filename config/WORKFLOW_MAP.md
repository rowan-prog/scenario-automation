...
| **(2026-09-02) 로그라인·트리트먼트·러프 기획안을 어떻게 쓰는지 — 현장 호평 T1 페이지 3편 제공** | 규격을 실물 없이 숫자로 적어 매번 반대편 벽으로 튀었다(45~70자 규격으로는 승인된 세 편이 전부 FAIL) | **모델 = `config/50_t1_page_exemplars.md`**(세 편 verbatim + 해부) · `50_logline_standard` §3·§4-1·§8 정정(110~175자·정본 2형·러프 기획안 = 페이지 1:1) · `/rough-proposal` 템플릿 교체 · lint 60~185 · 전 작업공간 = `~/.claude/CLAUDE.md` |
...
| **구조·룰·워크플로우 개정** (표준·절차·skill·도구 신설) | 문서 2~4 + 도구 | **4** (직교 분석 ≤2 · Fable 설계 1 · 절차 1) | **500k** | **30분** | 실물 정독 = 메인 · 분석은 직교 2기까지 · 설계 = Fable 1기 · **도구·빌더 구현은 메인이 직접**(게이트 명세가 이미 확정돼 있으면 위임 왕복이 더 비싸다) |

> **구조 개정 실측 (2026-09-01 · 피칭 페이지 신설):** 상한이 없어서 subagent 8기 · **142만 토큰 · 약 1시간**이 나갔다. 낭비 셋 = ①같은 파일 3개에 분석 4기 병렬(어휘 census 92k는 산출 미사용) ②린트 도구 위임 323k·29분 후 임계값 4건을 메인이 재보정 ③빌더 위임이 한도 초과로 폐기된 뒤 메인이 몇 분 만에 작성. 위 상한은 그 실측에서 낭비를 뺀 값 = **제안값이니 사용자가 덮어쓰면 그 값이 정본.**
