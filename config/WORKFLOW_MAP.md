...
| 라우팅이 흐리다 · 단일 agent로 돈다 | 절차가 문서에만 있고 호출 단위가 없었다 | **skill 10종**(작성 4 · 로그라인 1 · 검수 5) + workflow 1 · CLAUDE.md 룰 8 |
...
|---|---|---|---|---|
| 러프 기획안 | `/rough-proposal <원안>` | 로그라인 `/logline` · 셀링포인트 copy-candidate-generator(opus·low) · 트리트먼트 opus 별도 1기(medium) · 발상 idea-diverger(opus·low) | `pacing_lint --shortform` · `logline_lint` · 작업어 0 | `/plain-gate` → `/proposal-review` |
...
| 로그라인·타이틀 (러프 2단계 서브) | `/logline <작품>` · workflow `logline-round` | 생성 copy-candidate-generator(opus·low)×3 → `logline_lint` → plain-reader(haiku·low) → logline-auditor(sonnet·high) → 메인 선별 → 사용자 | 자수·문장·금지어 | (자체) |
...
- 라우팅 표·룰 7·8 = `CLAUDE.md` · 절차 = `.claude/skills/<name>/SKILL.md` 10종 · workflow = `.claude/workflows/logline-round.js`
