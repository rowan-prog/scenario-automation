...
3-1b-0. [price-defines-the-spec](feedback_price_defines_the_spec.md) — 🆕(2026-08-26 사용자) **가격이 규격을 정한다.** 우리 분당 단가가 영상 시장 최고가 → '고급감·완성도·품위'를 경쟁 축으로 잡으면 **자동 패배**(영화·OTT는 1/10 값에 100배). 이기는 축 = 그들이 배우·브랜드·심의 때문에 **구조적으로 못 파는 것** = 원초·금기·천박·윤리 이탈의 극단. 판정 = *"넷플릭스에 있어도 안 이상하다" = 결함 / "거긴 이런 걸 못 판다" = 정상.* ⚠️ 싸구려 허가 아님(틀린 건 여전히 결함) · 밀도는 오히려 더 높아야. 착지 = `00_vertical_dna` **조건 0 + S0-4(원초 욕망 9종)·S0-5(컨텍스트 제로 컷)·S0-6(후킹/락인/결제 세 관문)** · `20_review` **ⓞ 숏폼 7문** · `10_writing` §D-2-1 ⑦ 되감기 0. **락인은 첫 씬에서 걸린다 — 가중치 = 첫 컷 > 첫 씬 > 1화 엔딩(계약서 확정·보조) > 마지막 무료 엔딩. 내가 락인을 엔딩으로 옮겼다가 사용자에게 잡혔다(같은 날).**
3-1b-6. [budget-proportional-to-output](feedback_budget_proportional_to_output.md) — 🆕(2026-08-27 사용자 "1/10로 줄여도 모자라다 · 러프 기획안은 A4 1장") **절차 크기 = 산출물 크기.** 상한표 = `config/WORKFLOW_MAP.md` §0-1 · CLAUDE.md 룰 9 — **러프 기획안 agent ≤2·≤80k·≤3분("5분도 길다") / 로그라인 ≤2·≤40k·≤2분** · agent에 표준 정독 금지(규칙 인라인) · A4 한 장은 메인이 직접 본다(체크리스트·sonnet 판정 유닛 = 플랫폼·발송본 전용) · 작성 유닛 1기가 통째로 · 후보 ≤10 · 재라운드 자동 금지 · 코퍼스 전량 정독은 agent에 시키지 않음. 29 v6 실측 12기·110만·45분 = 반면교사. **새 절차 설계 시 첫 질문 = 산출물 몇 장인가.**
3-1b-4. [logline-catalog-slot-corpus](feedback_logline_catalog_slot_corpus.md) — 🆕(2026-08-27 사용자 "너가 하도 로그라인을 제대로 못써서 예시를 준다") **로그라인은 하나 — 러프 기획안 안의 그 문장이 플랫폼 페이지(≤70 words)에 그대로 실린다. 실물 179건 = `config/50_logline_corpus.md` · 실측 중앙값 55자·1~2문장 96%·인명 0~1.** 어제 만든 "13번 길이 4문장·150자"는 로그라인 규격이 아니었다(29번 v5 195자 = 다섯 번째 반려) — "기획안용 긴 로그라인" 갈래 없음. **7요소(관계/쾌감/핵심트롭/갈등구조/위기/능력/사이다)가 이름으로 한눈에 · 중학생 · 꼼꼼히 읽어도 띄엄띄엄 읽어도 재밌다.** 표준 = `config/50_logline_standard.md`(골격 5형·허용 = 코퍼스 굳은 말만) · 기계 `logline_lint.py` · 절차 `/logline`(생성 opus·low ×3 → lint → plain-reader → logline-auditor sonnet·high → 선별) · **메인 직접 작성 금지.** 4단 문단은 폐기('도입 문단'으로 살려뒀다가 사용자 지적으로 삭제 — 옛 룰을 새 이름으로 살리는 갈래 금지).
3-1b-5. [three-readers-rewind-zero-gate](feedback_three_readers_rewind_zero_gate.md) — 🆕(2026-08-27 사용자) **기획안·대본은 나(사용자)에게 보여주려고 쓰는 게 아니다 — 꼼꼼히 읽는 작업자 / 대충 읽는 작업자 / 이해 못 하는 사람이 읽는다.** 되감기 0(원관념 해독 한 번 더 = 결함)은 룰 문장으로는 안 고쳐졌다("수십번 수백번 말한 것들도 지침 업데이트하였다고만 하고 끔찍하게 이뤄진다") → **게이트**로: 한국어 산출물 전부 발송 전 `plain-reader`(haiku·low) 재진술 `못 함` 0 = `/plain-gate` · CLAUDE.md 룰 7. 내 자가 판정 무효. + 단일 agent 금지·effort 정책(창의 low/논리 high/반복 sonnet·haiku)·작업 모델 opus/sonnet/haiku만·검수의 검수(`review-auditor`·`proposal-spec-auditor`·`adaptation-fidelity-auditor`) = `20_review` §1④ · 룰 8. **skill 10종 = 작성 4(`/rough-proposal` `/platform-proposal` `/adapt` `/write-episodes`) + `/logline` + 검수 5 — 권역마다 작성·검수 한 쌍.**
3-1b-7. [fix-by-deleting-not-explaining](feedback_fix_by_deleting_not_explaining.md) — 🆕(2026-08-31 사용자 "니 논문 써서 재미 없는 거야") **이해 안 되는 자리 = 설명 덧대지 말고 그 자리를 없앤다.** 판정 = "이 문장 없으면 사람이 찾을까?" 안 찾으면 내 불안. 원작에 있어도 뜻 안 서면 버린다. 규칙은 한 번만 가르친다.
3-1b-8. [names-must-survive-ai-dubbing](feedback_names_must_survive_ai_dubbing.md) — 🆕(2026-09-01 사용자 "AI 더빙 시 오류가 안 나도록") **인명·사명 4검사 = 흔한가 / 철자→발음 경로 하나인가 / 발화 언어로 동음 상용어 없는가(Hale↔hail) / 인물끼리 첫소리 갈리는가.** 한국어 표기만 보고 고르다 3연속 실패.
3-1b-3. [proposal-prose-five-checks](feedback_proposal_prose_five_checks.md) — 🆕(2026-08-26 T1 반려 실증) **기획 프로즈 5검사 = 초등 어휘 · 은유 동사 0("침대를 사수") · 누가 누구에게("누가 누굴 구했는데?" — 주어 증발 0이지 문장 길이 검사 아님) · 능력 한 줄(카운트다운 X) · 원작에 없는 출처·세계 라벨 발명 0(용 조상·보험 왕조 → 기대불일치).** 네 지적의 뿌리 하나 = 원작이 설명 없이 굴러가는 걸 개연성·셀링 명분으로 덧칠. **+ 2차 교정: 설정이 아니라 문장 — 로그라인 4문장 골격(모욕 대사→당한 것→반전 실물→페이오프+장르 이름표) · 트리트먼트 = 첫 문장 못 박기·컷 재현 금지·몸이 어떻게 되는지·따옴표 1~2줄·마지막 = 훅(26 v18 §0-⑤). 반려가 오면 먼저 문장을 보라.** **+ 3차(같은 날): 직관 ≠ 짧게 끊기 — 연결어미 캡을 룰로 적었다가 로그라인이 토막 나 반려("짧고 끊어 치면 직관적이라고 착각한 듯"). 모델 13·26은 문장이 길고 연결어가 산다. 교정을 숫자 캡으로 바꿔 적는 버릇 금지 — 모델 문장 verbatim으로.** **+ 4차: 로그라인 ≠ 줄거리 요약(v4 = 8화 비트 열 개 사슬 400자 → 반려) — 13번 길이 150자·사건 1+반전 1+페이오프 1. 교정마다 반대편 벽으로 튐 = 룰 새로 짜지 말고 모델 옆에 놓고 길이·사건 수 맞출 것.** 본문 = `10_writing` §D-2-1 ⑧ · phase_p 대원칙 3-1 · 재작성 = `projects/29_sees_your_last_day/`(v3).
3-1b-2. [proposal-is-the-writers-spec](feedback_proposal_is_the_writers_spec.md) — 🆕(2026-08-26 사용자) **기획안 = 외부 작가의 스펙. 내가 원칙을 못 잡고 쓰면 작가가 그대로 쓰고 그 대본은 검수로 못 돌린다.** 발송 전 `20_review` §1③ 통과 증명(7문 판정+인용 · S0 세 컷 · 인물별 시청자 전지 채널표 · 화별 욕망 한 단어) 없이 발송 금지 = phase_p 기계 게이트 ⑨. 새 원칙은 진행 중 기획안에 소급.
3-1b-1. [viewer-omniscience-four-channels](feedback_viewer_omniscience_four_channels.md) — 🆕(2026-08-26 사용자) **인물끼리는 몰라도 시청자는 관계·목표·동기·정체를 첫 행동 전에 안다.** 채널 4 = 네임카드(정체·관계 기본값·히트작 실측 "백승 그룹의 상속자, 백선우.") / 주인공 몰래의 밀담(목표·동기 1순위 — 정보+분노 한 컷) / VO·독백 / 회상·INSERT. 미스터리화(인물이 모르니 시청자도 모르게) = 서스펜스 아니라 추리극. 예외 = 봉인 카드의 대반전 *내용*·한쪽 *내면*뿐, 존재는 알린다. 착지 = DNA 조건 1-1 · `10_writing` §C-5-3 + §B 네임카드 필수화 · `20_review` 7문 ⑦.
