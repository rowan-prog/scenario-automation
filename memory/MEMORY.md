...
3-1b-4. [logline-catalog-slot-corpus](feedback_logline_catalog_slot_corpus.md) — 🆕(2026-08-27 사용자 "너가 하도 로그라인을 제대로 못써서 예시를 준다") **로그라인 = 플랫폼 카탈로그 슬롯(한국어 ≤70 words) — 실물 179건 = `config/50_logline_corpus.md` · 실측 중앙값 55자·1~2문장 96%·인명 0~1.** 어제 만든 "13번 길이 4문장·150자"는 이 슬롯 규격이 아니었다(29번 v5 195자 = 다섯 번째 반려). **7요소(관계/쾌감/핵심트롭/갈등구조/위기/능력/사이다)가 이름으로 한눈에 · 중학생 · 꼼꼼히 읽어도 띄엄띄엄 읽어도 재밌다.** 표준 = `config/50_logline_standard.md`(골격 5형·허용 = 코퍼스 굳은 말만) · 기계 `logline_lint.py` · 절차 `/logline`(생성 opus·low ×3 → lint → plain-reader → logline-auditor sonnet·high → 선별) · **메인 직접 작성 금지.** 4단 문단은 '도입 문단'으로 개명.
