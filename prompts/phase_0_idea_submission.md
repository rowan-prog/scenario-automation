...
- 각 아이디어는 시청자가 기다릴 **하나의 핵심 질문**을 먼저 세운다.
- 좋은 아이디어는 사건을 여러 개 벌이는 것이 아니라, 한 상황이 더는 버틸 수 없을 만큼 커진다.
- EP1-3은 기본 쾌감을 증명하고, EP8은 정보 공개가 아니라 선택·명령·접촉·폭로·처벌 같은 **다음 행동의 결과** 직전에 멈춘다.
- 반복 보상형 아이디어라면 원초 보상축이 한 줄로 잡히는가? 같은 모욕/참교육 복사가 아니라, 같은 쾌감이 새 무대·새 증인·새 증거로 반복 소비되는 구조인지 확인한다.
- 남성향은 약함을 오래 끌지 않는다. 초반부터 첫 승리나 첫 권한을 주고, 이후 더 큰 소유·지배·통제·매력 여성·영역으로 같은 획득축을 확장한다.
